...
export class PdfHandler {
	...
	public getRentAgreement(emailSetting: EmailSetting, emailOrder: EmailOrder, emailUser: EmailUser): Promise<EmailAttachment> {
		const receiptWithAgreementHtml = this._templateCompiler.getReceiptWithAgreementHtml(this._emailTemplateConfig, this._emailHandler.createEmailTemplateInput(emailSetting, emailOrder, emailUser));
		return this.createPdf(receiptWithAgreementHtml);
	}

	public getOrderReceipt(emailSetting: EmailSetting, emailOrder: EmailOrder, emailUser: EmailUser): Promise<EmailAttachment> {
		const orderReceiptHtml = this._templateCompiler.getReceiptPdf(this._emailTemplateConfig, this._emailHandler.createEmailTemplateInput(emailSetting, emailOrder, emailUser));
